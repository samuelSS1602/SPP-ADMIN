...
function loadGuests() {
    ...
    data.guests.forEach(guest => {
        ...
        const photoBtn = currentBookingId
            ? `<button class="btn-primary" onclick="showGuestVisitHistory('${guest.name}', '${guest.phone}')" style="padding: 6px 10px; font-size: 11px; background: #4F46E5;"><i class="fas fa-camera"></i> View All</button>`
...
async function showGuestVisitHistory(guestName, guestPhone) {
    // Find all bookings for this guest
    const guestBookings = data.bookings.filter(b => 
        b.guestName === guestName || b.guestPhone === guestPhone
    ...
    const modal = document.getElementById('guestVisitHistoryModal');
    const title = document.getElementById('guestVisitHistoryTitle');
    const content = document.getElementById('guestVisitHistoryContent');

    title.textContent = `${guestName}`;
    
    // Show loading state
    content.innerHTML = '<div style="text-align: center; padding: 30px;"><i class="fas fa-spinner fa-spin"></i> Loading booking history...</div>';
    modal.style.display = 'flex';

    // Fetch photos from Firebase for all bookings
    for (let booking of guestBookings) {
        ...
        // Try to fetch from Firebase if not cached
        if ((!localCustomer || !localIdProof) && typeof firebaseDb !== 'undefined' && firebaseDb) {
            try {
                const doc = await firebaseDb.collection('booking_photos').doc(String(booking.id)).get();
                if (doc.exists) {
                    const picData = doc.data();
                    if (picData.customerPhoto && !localCustomer) {
                        booking.customerPhotoUrl = picData.customerPhoto;
                    }
                    if (picData.idProofPhoto && !localIdProof) {
    ...
    guestBookings.forEach((booking, index) => {
        ...
        html += `
            <div style="border: 2px solid #3b82f6; border-radius: 8px; padding: 15px; background: ${booking.status === 'completed' ? '#f0fdf4' : '#fffbeb'}; cursor: pointer; transition: all 0.3s;" 
                 onmouseover="this.style.boxShadow='0 4px 12px rgba(59, 130, 246, 0.3)'; this.style.transform='translateY(-2px)';" 
                 onmouseout="this.style.boxShadow='none'; this.style.transform='translateY(0)';"
                 onclick="showIndependentBooking('${booking.id}', '${guestName}')">
                
                <div style="display: flex; justify-content: space-between; align-items: center;">
                    <div style="flex: 1;">
                        <h4 style="margin: 0 0 5px 0; color: #ef4444; font-size: 18px; font-weight: 700;">${booking.id}</h4>
                        <p style="margin: 0; font-size: 12px; color: var(--text-light);">
                            ${formatDate(booking.checkIn)} to ${formatDate(booking.checkOut)} • ${days} night${days !== 1 ? 's' : ''}
                        </p>
                        <p style="margin: 5px 0 0 0; font-size: 13px; color: #1e40af; font-weight: 600;">
                            <i class="fas fa-bed"></i> ${booking.rooms && booking.rooms.length > 0 ? booking.rooms.map(r => r.roomName).join(', ') : booking.roomName}
                        </p>
                    </div>
                    <div style="text-align: right;">
                        <span class="status-badge ${booking.status}" style="padding: 6px 12px; font-size: 11px; font-weight: 600; margin-bottom: 8px; display: inline-block;">
                            ${capitalizeFirst(booking.status)}
                        </span>
                        <p style="margin: 8px 0 0 0; font-size: 14px; color: #1e40af; font-weight: 700;">₹${formatNumber(total)}</p>
                    </div>
                </div>
                
                <div style="margin-top: 10px; font-size: 12px; color: var(--primary-brand); font-weight: 600;">
                    <i class="fas fa-click"></i> Click to view details
                </div>
            </div>
        `;
    });
...
function closeGuestVisitHistoryModal() {
    const modal = document.getElementById('guestVisitHistoryModal');
    if (modal) modal.style.display = 'none';
}

async function showIndependentBooking(bookingId, guestName) {
    const booking = data.bookings.find(b => b.id == bookingId);
    ...
    // Create and show independent booking modal
    const modal = document.getElementById('independentBookingModal');
    ...
    document.getElementById('independentBookingGuestName').textContent = booking.guestName;
    document.getElementById('independentBookingId').textContent = booking.id;
    document.getElementById('independentBookingStatus').textContent = capitalizeFirst(booking.status);
    document.getElementById('independentBookingStatus').className = `status-badge ${booking.status}`;
    
    document.getElementById('independentBookingCheckIn').textContent = formatDate(booking.checkIn);
    document.getElementById('independentBookingCheckOut').textContent = formatDate(booking.checkOut);
    document.getElementById('independentBookingNights').textContent = `${days} night${days !== 1 ? 's' : ''}`;
    
    document.getElementById('independentBookingRoom').textContent = booking.rooms && booking.rooms.length > 0 ? booking.rooms.map(r => r.roomName).join(', ') : booking.roomName;
    document.getElementById('independentBookingRate').textContent = `₹${formatNumber(booking.roomRate)}/night`;
    document.getElementById('independentBookingTotal').textContent = `₹${formatNumber(total)}`;
    document.getElementById('independentBookingBalance').textContent = `₹${formatNumber(balance)}`;
    document.getElementById('independentBookingBalance').style.color = balance > 0 ? '#ef4444' : '#27ae60';

    // Set photos
    const customerPhotoDiv = document.getElementById('independentBookingCustomerPhoto');
    const idProofDiv = document.getElementById('independentBookingIdProof');

    if (customerPhoto) {
        customerPhotoDiv.innerHTML = `<img src="${customerPhoto}" alt="Customer Photo" style="width: 100%; height: 280px; object-fit: contain; background: #f8fafc; border-radius: 6px; border: 1px solid #e2e8f0; cursor: pointer;" onclick="expandPhoto(this)">`;
    } else {
        customerPhotoDiv.innerHTML = '<div style="text-align: center; padding: 40px; background: #f8fafc; border-radius: 6px; color: var(--text-light);"><i class="fas fa-image"></i> No Guest Photo</div>';
    }

    if (idProofPhoto) {
        idProofDiv.innerHTML = `<img src="${idProofPhoto}" alt="ID Proof" style="width: 100%; height: 280px; object-fit: contain; background: #f8fafc; border-radius: 6px; border: 1px solid #e2e8f0; cursor: pointer;" onclick="expandPhoto(this)">`;
    } else {
        idProofDiv.innerHTML = '<div style="text-align: center; padding: 40px; background: #f8fafc; border-radius: 6px; color: var(--text-light);"><i class="fas fa-image"></i> No ID Proof</div>';
    }

    // Close the history modal and open independent modal
    document.getElementById('guestVisitHistoryModal').style.display = 'none';
    modal.style.display = 'flex';
}

function closeIndependentBookingModal() {
    document.getElementById('independentBookingModal').style.display = 'none';
    // Reopen guest history modal
    document.getElementById('guestVisitHistoryModal').style.display = 'flex';
}
...
window.onclick = function (event) {
    ...
    const guestVisitHistoryModal = document.getElementById('guestVisitHistoryModal');
    const independentBookingModal = document.getElementById('independentBookingModal');
    const allPhotosModal = document.getElementById('allPhotosModal');
    ...
    if (event.target == guestVisitHistoryModal) closeGuestVisitHistoryModal();
    if (event.target == independentBookingModal) closeIndependentBookingModal();
    if (event.target == allPhotosModal) closeAllPhotosModal();
}

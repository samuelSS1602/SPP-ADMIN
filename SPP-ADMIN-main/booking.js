...
async function viewBookingPhotos(bookingId) {
    ...
    const updateCustomerBtn = document.getElementById('updateCustomerPhotoBtn');
    const updateIdProofBtn = document.getElementById('updateIdProofPhotoBtn');
    ...
    // Disable update buttons if guest has checked out
    if (booking.status === 'completed') {
        if (updateCustomerBtn) {
            updateCustomerBtn.disabled = true;
            updateCustomerBtn.style.opacity = '0.5';
            updateCustomerBtn.style.cursor = 'not-allowed';
            updateCustomerBtn.title = 'Cannot update - Guest has checked out';
        }
        if (updateIdProofBtn) {
            updateIdProofBtn.disabled = true;
            updateIdProofBtn.style.opacity = '0.5';
            updateIdProofBtn.style.cursor = 'not-allowed';
            updateIdProofBtn.title = 'Cannot update - Guest has checked out';
        }
    } else {
        // Re-enable buttons for active bookings
        if (updateCustomerBtn) {
            updateCustomerBtn.disabled = false;
            updateCustomerBtn.style.opacity = '1';
            updateCustomerBtn.style.cursor = 'pointer';
            updateCustomerBtn.title = 'Update Photo';
        }
        if (updateIdProofBtn) {
            updateIdProofBtn.disabled = false;
            updateIdProofBtn.style.opacity = '1';
            updateIdProofBtn.style.cursor = 'pointer';
            updateIdProofBtn.title = 'Update Photo';
        }
    }
...
window.handleUpdateCustomerPhoto = function(inputElement) {
    ...
    reader.onload = function(e) {
        ...
        // Prevent photo update if guest has checked out
        if (booking.status === 'completed') {
            alert('Cannot update photo. Guest has already checked out. Original booking photos are preserved.');
            inputElement.value = '';
            return;
        }
